...
if(isset($_POST['upload_product'])){
  ...
  $executeSat = mysqli_query($koneksi, "SELECT * FROM tabel_kategori_barang WHERE kd_kategori = '$kategori'");
  while ($b   = mysqli_fetch_array($executeSat)) {
  	$inesial  = $b['inesial'];
  }

	$random     = rand();
	$kode 			= $inesial.'-0'.substr($random,0,3);
				...
				echo '<script>
	                setTimeout(function() {
	                    Swal.fire({
	                        icon: "success",
	                        tittle: "BERHASIL.",
	                        text: "Produk berhasil di simpan.",
	                    }).then(function() {
	                        history.back();
	                    });
	                }, 1);

        ...
        <div class="col-lg-6 col-6">
            <fieldset>
                ...
                <div class="input-group">
                    <input type="text" class="form-control" id="kodebarang" name="kodebarang" autofocus="autofocus" placeholder="Scan Here!" aria-describedby="button-addon2" onkeyup="check_data()">
                    <div class="input-group-append" id="button-addon2">
                        <button class="btn btn-primary rounded-0" type="button" onclick="startBarcodeScanner()"><i class="fas fa-qrcode"></i></button>
                    </div>
                </div>
            </fieldset>
        </div>
...
<script>
    function startBarcodeScanner() {
        Quagga.init({
            inputStream: {
                name: "Live",
                type: "LiveStream",
                target: document.querySelector('#kodebarang'),
                constraints: {
                    width: 480,
                    height: 320,
            ...
            decoder: {
                readers: ["ean_reader", "upc_reader", "code128_reader", "code39_reader", "codabar_reader"]
            }
        }, function (err) {
            if (err) {
                console.error(err);
        ...
        Quagga.onDetected(function (result) {
            document.getElementById('kodebarang').value = result.codeResult.code;
            Quagga.stop();
        });
    }
    ...
    function processTransferPayment() {
        ...
        alert('Processing transfer payment...');
        // You can also use AJAX to communicate with the server and update the payment status
        // Example: Make an AJAX request to a server-side script to update the payment status
        // $.ajax({
        //     url: 'your_server_script.php',
        //     method: 'POST',
        //     data: { action: 'processTransferPayment', /* other data */ },
...
<button type="button" class="btn btn-primary rounded-0 mr-1 mb-1 ml-1" onclick="printReceipt()">Print</button>
<button type="button" class="btn btn-warning rounded-0 mr-1 mb-1" style="font-size: 10px; color: white;" onclick="processTransferPayment()">Pembayaran<br>Transfer</button>
<button type="button" class="btn btn-danger rounded-0 mr-1 mb-1" onclick="cancelTransaction()">Batal</button>
